...
class Worker(QThread):
    ...
    def run(self):
        ...
        if packets:
            alerts, analyzed_data = analyze_packets(packets)
            if analyzed_data:
            ...
            if alerts:
                for alert in alerts:
                    self.error.emit(alert)
        else:
            self.error.emit("No packets captured.")
        self.running = False  # 标记运行结束
        # 循环结束后处理所有数据包
        if self.all_packets:
            analyzed_data = analyze_packets(self.all_packets)
            self.save_results_to_db(analyzed_data)
            formatted_data = self.format_data(analyzed_data)
            self.all_data_captured.emit(formatted_data)  # 确保这一行代码无误
            # try:
    ...
    def format_data(self, data):
        ...
        if 'protocols' in data:
            results.append(("Protocols Usage", "", "Count of each protocol layer"))
            for key, value in data['protocols'].items():
                results.append((key, value, "Protocol layer"))

        # 处理层序列
        if 'protocol_layers' in data:
            results.append(("Protocol Layers", "", "Layer sequences"))
            for layers, count in data['protocol_layers'].items():
                results.append((' -> '.join(layers), count, "Flow of layers"))

        # 处理源 IP 使用情况
        if 'ip_src' in data:
            results.append(("Source IP Usage", "", "Usage count of source IPs"))
            for key, value in data['ip_src'].items():
                results.append((key, value, "Source IP"))

        # 处理目标 IP 使用情况
        if 'ip_dst' in data:
            results.append(("Destination IP Usage", "", "Usage count of destination IPs"))
            for key, value in data['ip_dst'].items():
                results.append((key, value, "Destination IP"))

        # 处理 TCP 标志
        if 'tcp_flags' in data:
            results.append(("TCP Flags Usage", "", "Occurrence of TCP flags"))
            for key, value in data['tcp_flags'].items():
                results.append((key, value, "TCP Flag"))

        return results

    # def format_data(self, data):
    #     """将字典格式化为字符串"""
    #     result = [("Summary", "Count", "Details")]
    #     result.extend([
    #         ("Protocols Usage", "", ""),
    #         *[(key, value, "Protocol layer") for key, value in data['protocols'].items()],
    #         ("Protocol Layers", "", ""),
    #         *[(key, count, "Layer sequence") for key, count in data['protocol_layers'].items()],
    #         ("Source IP Usage", "", ""),
    #         *[(key, value, "Source IPs") for key, value in data['ip_src'].items()],
    #         ("Destination IP Usage", "", ""),
    #         *[(key, value, "Destination IPs") for key, value in data['ip_dst'].items()],
    #         ("TCP Flags Usage", "", ""),
    #         *[(key, value, "TCP Flags") for key, value in data['tcp_flags'].items()]
    #     ])
        # result = []
...
class WindowsView(QWidget):
    ...
    def setupWorker(self):
        self.worker = Worker("Intel(R) Ethernet Connection (12) I219-V")  # 假设默认接口，需要根据实际情况设置
        self.worker.all_data_captured.connect(self.updateUI)
    ...
    def startMonitoring(self):
        ...
        if not hasattr(self, 'worker') or not self.worker.isRunning():
            ...
            QMessageBox.information(self, "Already running", "Monitoring is already in progress.")
            # self.worker = Worker(interface)
    ...
    def displayResults(self, results):
        if isinstance(results, pd.DataFrame):
            # 清除当前表格内容
            self.resultsTab.setRowCount(0)
            self.resultsTab.setColumnCount(len(results.columns))
            self.resultsTab.setHorizontalHeaderLabels(results.columns.tolist())

            # 添加新行
            for index, row in results.iterrows():
                row_pos = self.resultsTab.rowCount()
                self.resultsTab.insertRow(row_pos)
                for col_index, value in enumerate(row):
                    self.resultsTab.setItem(row_pos, col_index, QTableWidgetItem(str(value)))
        else:
            QMessageBox.information(self, "Error", "No data to display.")
